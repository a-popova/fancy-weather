...
window.onload = () => {
  ...
  function renderWeatherIcons(day, index) {
    ...
    document.querySelector(`.weatherForecast--day${index}--image`).style.backgroundImage = `url(dist/${iconURL})` || `url(fancy-weather/dist/${iconURL})`;
  }
  ...
  function extractForecastData(APIResponse) {
    ...
    forecast.iconURL = `url(dist/${iconURL})` || `url(fancy-weather/dist/${iconURL})`;
